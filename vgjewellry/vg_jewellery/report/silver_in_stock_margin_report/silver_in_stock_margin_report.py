...
def execute(filters=None):
    ...
    for branch in branch_array:
        ...
        condition=" TDate>'"+ yesterday_str+"' and ItemTradMstID in (4006,4005,4004,4003,4001)";
        rate_res=get_sql_server_data(branch,table,columns,condition)
        logger.info(f"{rate_res}")
        rate_master= {}
        for rr in rate_res:
        ...
        condition="VouType='ST'  and  ItemTradMstId in (4004,4003,4005,6005)  and ItemMstID not in (4002,4001)  and LabelNo not like 'O%' and LabelNo in (select LabelNo from LabelBalance where itemTradMstId in (4004,4003,4005,6005)) ";    
        condition="VouType='ST'  and   LabelNo not like 'O%' and LabelNo in (select LabelNo from LabelBalance where itemTradMstId in (4004,4003,4005,6005)) ";    
        ...
        for slr in select_label_res:
            ...
            VouDate = datetime.now().strftime("%Y-%m-%d")
