frappe.pages['ideal-stock-for-diam'].on_page_load = function(wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: 'Ideal Stock For Diamond',
		single_column: true
	});
	// --- CSS Styling ---
	const style = `
	...
	$("#set_target_stock").on("click",function(){
		...
		loadingDiv()
		frappe.call({
			method: "vgjewellry.diamond_ideal_stock_api.set_target_stock", 
			args: {
	...
	frappe.call({
		...
		frappe.call({
                        method: "vgjewellry.diamond_ideal_stock_api.get_ideal_stock_date",
                        callback: function (res) {
				let r=res.message[0]
		...
		frappe.call({
                        method: "vgjewellry.diamond_ideal_stock_api.calculate_ideal_stock",
                        callback: function (r) {
				stdPivotUi(r.message);
				unloadingicon()
                        }
...
function loadingicon() {
	...
	$("#fetch_btn").on("click", function () {
		...
		frappe.call({
			method: "vgjewellry.diamond_ideal_stock_api.get_ideal_stock", 
			args: {
				from_date: from_iso,  
				to_date: to_iso
			},
